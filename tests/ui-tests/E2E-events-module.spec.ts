...
test.describe.serial('Verify new event creation, booking and booking deletion', async () => {
    ...
    test('Verify user is able to book a newly created event', {
        tag: ['@UI', '@UAT'],
        annotation: {
            type: 'Test Case Link',
            description: 'https://eventhub.rahulshettyacademy.com/'
        ...
        await eventBookingPage.bookEventTicket(newEventData.event_booking_number_of_tickets, newEventData.event_booking_full_name, newEventData.event_booking_email, newEventData.event_booking_phone_number);
        ...
        const totalPrice = parseInt(newEventData.new_event_price) * parseInt(newEventData.event_booking_number_of_tickets);
        ...
        expect(parseInt(seatsAfterBooking) === (parseInt(seatsBeforeBooking) - parseInt(newEventData.event_booking_number_of_tickets))).toBeTruthy();

    })

    test('Verify that the user is able to verify booking details under my bookings', {
        tag: ['@UI', '@UAT'],
        annotation: {
            type: 'Test Case Link',
            description: 'https://eventhub.rahulshettyacademy.com/'
    ...
    test('Verify that user is able to verify the booking details in view details section', {
        tag: ['@UI', '@UAT'],
        annotation: {
            type: 'Test Case Link',
            description: 'https://eventhub.rahulshettyacademy.com/'
    ...
    test('Verify that user is able to verify the refund eligibility under booking details in view details section and then cancel', {
        tag: ['@UI', '@UAT'],
        annotation: {
            type: 'Test Case Link',
            description: 'https://eventhub.rahulshettyacademy.com/'
        ...
        if (parseInt(newEventData.event_booking_number_of_tickets) > 1) {
            await expect(myBookingsPage.refundResultMsg).toContainText(` Group bookings (${newEventData.event_booking_number_of_tickets} tickets) are non-refundable.`)
    ...
    test('Verify user is able to delete a newly created event', {
        tag: ['@UI', '@UAT'],
        annotation: {
            type: 'Test Case Link',
            description: 'https://eventhub.rahulshettyacademy.com/'
